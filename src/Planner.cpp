...
void Planner::jointPoseAccCB(const nav_msgs::Odometry::ConstPtr & rbtOdomMsg, 
                ...
                for (int j = 0; j < gapTube->size(); j++) 
                {
                    ...
                    if (runGoToGoal) 
                    {
                        ...
                        goToGoalTraj = gapTrajGenerator_->processTrajectory(goToGoalTraj, 
                                                                            ...
                                                                            currVel,
                                                                            false);
                        trajEvaluator_->evaluateTrajectory(goToGoalTraj, goToGoalPoseCosts, goToGoalTerminalPoseCost, futureScans, scanIdx, gap);
                    ...
                    if (gap->isAvailable())
                    {
                        ...
                        pursuitGuidanceTraj = gapTrajGenerator_->processTrajectory(pursuitGuidanceTraj, 
                                                                                    ...
                                                                                    currVel,
                                                                                    false);
                        trajEvaluator_->evaluateTrajectory(pursuitGuidanceTraj, pursuitGuidancePoseCosts, pursuitGuidanceTerminalPoseCost, futureScans, scanIdx, gap);
                                        ...
                                        const std::vector<sensor_msgs::LaserScan> & futureScans) 
    {
        try
        {
            ...
            trajEvaluator_->evaluateTrajectory(idlingTrajectory, idlingPoseCosts, idlingTerminalPoseCost, futureScans, 0, 0);
                                                ...
                                                const bool & isCurrentGapFeasible) // bool isIncomingGapAssociated,
    {
        ...
        try
        {
            ...
            trajEvaluator_->evaluateTrajectory(incomingTraj, incomingPathPoseCosts, incomingPathTerminalPoseCost, futureScans, 0, incomingGap);
            ...
            trajEvaluator_->evaluateTrajectory(reducedCurrentTraj, reducedCurrentPathPoseCosts, reducedCurrentPathTerminalPoseCost, futureScans, updatedCurrentPathPoseIdx, incomingGap);
            ...
            ROS_INFO_STREAM_NAMED("GapTrajectoryGeneratorV2", "    reduced current trajectory received a subcost of: " << reducedCurrentPathSubCost);
    ...
    void Planner::runPlanningLoop(Trajectory & chosenTraj, int & trajFlag) 
    {
        ...
        goalVisualizer_->drawGapGoals(gapTubes);
        ...
        if (trajFlag != NONE) 
        {
            ...
            if (trajFlag == GAP) // comparing to traj within one of the gaps
            {
                ...
                {
                    ROS_WARN_STREAM_NAMED("Planner", "       lowest cost gap trajectory index is negative");
                    chosenTraj = Trajectory();
                    trajFlag = NONE;
                    return;
                }
            ...
            } else if (trajFlag == UNGAP) // comparing to un-gap traj
            {
                ...
                if (lowestCostTrajIdx < 0 || lowestCostTrajIdx >= ungapTrajs.size())
                {
            ...
            } else if (trajFlag == IDLING)
            {
                ...
                if (lowestCostTrajIdx < 0 || lowestCostTrajIdx >= idlingTrajs.size())
                {
