...
void Planner::jointPoseAccCB(const nav_msgs::Odometry::ConstPtr & rbtOdomMsg, 
            ...
            for (int j = 0; j < gapTube->size(); j++) 
            {
                ...
                if (runGoToGoal) 
                {
                    ...
                    goToGoalTraj = gapTrajGenerator_->processTrajectory(goToGoalTraj, 
                                                                        ...
                                                                        currVel,
                                                                        false);
                    trajEvaluator_->evaluateTrajectory(goToGoalTraj, goToGoalPoseCosts, goToGoalTerminalPoseCost, futureScans, gap);
                ...
                if (gap->isAvailable())
                {
                    ...
                    pursuitGuidanceTraj = gapTrajGenerator_->processTrajectory(pursuitGuidanceTraj, 
                                                                                ...
                                                                                currVel,
                                                                                false);
                    trajEvaluator_->evaluateTrajectory(pursuitGuidanceTraj, pursuitGuidancePoseCosts, pursuitGuidanceTerminalPoseCost, futureScans, gap);
                                                ...
                                                const std::vector<sensor_msgs::LaserScan> & futureScans) // bool isIncomingGapAssociated,
    {
        ...
        trajEvaluator_->evaluateTrajectory(incomingTraj, incomingPathPoseCosts, incomingPathTerminalPoseCost, futureScans, incomingGap);
        ...
        trajEvaluator_->evaluateTrajectory(reducedCurrentTraj, reducedCurrentPathPoseCosts, reducedCurrentPathTerminalPoseCost, futureScans, incomingGap);
    ...
    void Planner::runPlanningLoop(Trajectory & chosenTraj, int & trajFlag) 
    {
        ...
        if (trajFlag != NONE) 
        {
            ...
            if (trajFlag == GAP) // comparing to traj within one of the gaps
            {
                ...
                if (lowestCostTrajIdx < 0 || lowestCostTrajIdx >= gapTrajs.size() || lowestCostTrajIdx >= feasibleGaps.size())
                {
                    ROS_WARN_STREAM_NAMED("Planner", "       lowest cost gap trajectory index out of bounds");
                    chosenTraj = Trajectory();
                    trajFlag = NONE;
                    return;
                }
