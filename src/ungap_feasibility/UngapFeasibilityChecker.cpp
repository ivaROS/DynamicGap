...
{
    void UngapFeasibilityChecker::updateEgoCircle(boost::shared_ptr<sensor_msgs::LaserScan const> scan) 
    {
        scan_ = scan;
    }

    bool UngapFeasibilityChecker::pursuitGuidanceAnalysis(Ungap * ungap)
    {
        ...
        {
            // parallel navigation
            return parallelNavigationFeasibilityCheck(ungap);
        }
    }

    bool UngapFeasibilityChecker::parallelNavigationFeasibilityCheck(Ungap * ungap)
    {
        ...
        Eigen::Vector4f leftGapState = ungap->getLeftUngapPt()->getModel()->getGapState();
        Eigen::Vector4f rightGapState = ungap->getRightUngapPt()->getModel()->getGapState();
        ...
        Eigen::Vector2f p_target = ungap->getGoal()->getOrigGoalPos();  // (gap->goal.x_, gap->goal.y_);

        // set target velocity to mean of left and right gap points
        Eigen::Vector2f v_target = ungap->getGoal()->getOrigGoalVel(); // (gap->goal.vx_, gap->goal.vy_);
        ...
        {
            ROS_INFO_STREAM_NAMED("GapFeasibility", "                    ungap is not feasible! t_intercept: " << t_intercept_goal); 
            return false;
        } else
        {
            ROS_INFO_STREAM_NAMED("GapFeasibility", "                    ungap is feasible!  t_intercept_goal: " << t_intercept_goal); 
            ...
            } else if (t_intercept_goal > cfg_->traj.integrate_maxt) //      Can happen when ego-robot not fast enough
            {
                terminalGoal = p_target + v_target * cfg_->traj.integrate_maxt;
            } else
            {
            ...
            // set gamma_rbt
            ungap->setTInterceptGoal(t_intercept_goal);
            ungap->setGammaInterceptGoal(terminalGoalTheta);

            ungap->getGoal()->setTermGoalPos(terminalGoal);
    ...
    void UngapFeasibilityChecker::parallelNavigationHelper(const Eigen::Vector2f & p_target_init, 
