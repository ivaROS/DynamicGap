...
namespace dynamic_gap 
{
    void DynamicGapConfig::loadRosParamFromNodeHandle(const std::string & name)
    {
        ...
        if (model == "tb2")
        {
            ...
            ros_throw_param_load(nh, "map_frame_id", map_frame_id);

            // nh.param("odom_frame_id", odom_frame_id, odom_frame_id);
            ros_throw_param_load(nh, "odom_frame_id", odom_frame_id);

            // nh.param("robot_frame_id", robot_frame_id, robot_frame_id);
            ros_throw_param_load(nh, "robot_frame_id", robot_frame_id);

            // nh.param("sensor_frame_id", sensor_frame_id, sensor_frame_id);
            ros_throw_param_load(nh, "sensor_frame_id", sensor_frame_id);
    
            odom_topic = "/odom"; // model + "/odom";
            ROS_INFO_STREAM_NAMED("Parameters", "odom_topic is: " << odom_topic);

            acc_topic = "/mobile_base/sensors/imu_data"; // model + "/acc";
            ROS_INFO_STREAM_NAMED("Parameters", "acc_topic is: " << acc_topic);

            scan_topic = "/mod_scan"; // model + "/scan";
            ROS_INFO_STREAM_NAMED("Parameters", "scan_topic is: " << scan_topic);

            odom_frame_id = model + "/odom";
            ROS_INFO_STREAM_NAMED("Parameters", "odom_frame_id is: " << odom_frame_id);

            robot_frame_id = model + "/base_link";
            ROS_INFO_STREAM_NAMED("Parameters", "robot_frame_id is: " << robot_frame_id);

            sensor_frame_id = model + "/hokuyo_link";
            ROS_INFO_STREAM_NAMED("Parameters", "sensor_frame_id is: " << sensor_frame_id);
