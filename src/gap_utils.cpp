...
namespace dynamic_gap {
    ...
    std::vector<dynamic_gap::Gap> GapUtils::hybridScanGap(boost::shared_ptr<sensor_msgs::LaserScan const> sharedPtr_laser, geometry_msgs::PoseStamped final_goal_rbt)
    {
        ...
        for (std::vector<float>::size_type it = 1; it < stored_scan_msgs.ranges.size(); ++it)
        {
            ...
            if (std::abs(scan_diff) > 0.2) 
            {
                ...
                if (scan_dist < max_scan_dist && last_scan < max_scan_dist) 
                {
                    ...
                    detected_gap.addLeftPOVInformation(it, scan_dist);
        ...
        if (prev_lgap) 
        {
            ...
            detected_gap.addLeftPOVInformation(last_scan_idx, last_scan_dist);
            detected_gap.setMinSafeDist(min_dist);
            //std::cout << "candidate last gap from (" << gap_ridx_pov << ", " << gap_rdist_pov << "), to (" << last_scan_idx << ", " << last_scan_dist << ")" << std::endl;
            if (detected_gap.LIdxPOV() - detected_gap.RIdxPOV() > 500 || detected_gap.get_dist_side() > 4 * cfg_->rbt.r_inscr) {
        ...
        if (raw_gaps.size() > 1)
        {
            ...
            //std::cout << "checking: " << raw_gaps[0].RIdxPOV() << " and " << raw_gaps[raw_gaps.size() - 1].LIdxPOV() << std::endl;
            if (raw_gaps[0].RIdxPOV() == 0 && raw_gaps[last_raw_gap_idx].LIdxPOV() == (stored_scan_msgs.ranges.size() - 1)) // Magic number?
            {
                // ROS_INFO_STREAM("bridging first and last gaps");
                // Both ends
                // currently only changing distances?
                float start_side_dist = raw_gaps[0].LDistPOV();
                int start_side_idx = raw_gaps[0].LIdxPOV();
                float end_side_dist = raw_gaps[last_raw_gap_idx].RDistPOV();
                int end_side_idx = raw_gaps[last_raw_gap_idx].RIdxPOV();
                // ROS_INFO_STREAM("original first gap: (" << raw_gaps[0].RIdxPOV() << ", " << raw_gaps[0].RDistPOV() << ") to (" << raw_gaps[0].LIdxPOV() << ", " << raw_gaps[0].LDistPOV()  << ")");
                // ROS_INFO_STREAM("original last gap: (" << raw_gaps[raw_gaps.size() - 1].RIdxPOV() << ", " << raw_gaps[raw_gaps.size() - 1].RDistPOV() << ") to (" << raw_gaps[raw_gaps.size() - 1].LIdxPOV() << ", " << raw_gaps[raw_gaps.size() - 1].LDistPOV()  << ")");
                ...
                // raw_gaps[raw_gaps.size() - 1].addLeftPOVInformation(511, result);
                // raw_gaps[0].setRDistPOV(result);
                // ROS_INFO_STREAM("revised first gap: (" << raw_gaps[0].RIdxPOV() << ", " << raw_gaps[0].RDistPOV() << ") to (" << raw_gaps[0].LIdxPOV() << ", " << raw_gaps[0].LDistPOV()  << ")");
                // ROS_INFO_STREAM("revised last gap: (" << raw_gaps[raw_gaps.size() - 1].RIdxPOV() << ", " << raw_gaps[raw_gaps.size() - 1].RDistPOV() << ") to (" << raw_gaps[raw_gaps.size() - 1].LIdxPOV() << ", " << raw_gaps[raw_gaps.size() - 1].LDistPOV()  << ")");

                //std::cout << "merging into last gap: (" << raw_gaps[last_raw_gap_idx].RIdxPOV() << ", " << raw_gaps[last_raw_gap_idx].RDistPOV() << ") to (" << raw_gaps[last_raw_gap_idx].LIdxPOV() << ", " << raw_gaps[last_raw_gap_idx].LDistPOV()  << ")" << std::endl;
                raw_gaps[last_raw_gap_idx].addLeftPOVInformation(start_side_idx, start_side_dist);
                raw_gaps.erase(raw_gaps.begin(), raw_gaps.begin() + 1);
                //std::cout << "revised first gap: (" << raw_gaps[0].RIdxPOV() << ", " << raw_gaps[0].RDistPOV() << ") to (" << raw_gaps[0].LIdxPOV() << ", " << raw_gaps[0].LDistPOV()  << ")" << std::endl;
            }
        }
    ...
    std::vector<dynamic_gap::Gap> GapUtils::addTerminalGoal(int final_goal_idx,
        ...
        for (dynamic_gap::Gap g : raw_gaps) {
            ...
            // ROS_INFO_STREAM("checking against: " << g.RIdxPOV() << " to " << g.LIdxPOV());
            if (final_goal_idx >= g.RIdxPOV() && final_goal_idx <= g.LIdxPOV()) {
                ROS_INFO_STREAM("final goal is in gap: " << g.RIdxPOV() << ", " << g.LIdxPOV());
                return raw_gaps;
            }
            gap_idx += 1;
        }
        ...
        detected_gap.addLeftPOVInformation(left_idx_pov, stored_scan_msgs.ranges.at(left_idx_pov));
        detected_gap.setMinSafeDist(min_dist);
            ...
            {   
                ...
                if (!mark_to_start) // if we have already found a mergable gap
                {
                    if (raw_gaps.at(i).isAxial()) // if gap is axial
                    {
                        ...
                        {
                            float curr_rdist = raw_gaps[i].LDistPOV();
                            int erase_counter = 0;
                            int last_mergable = -1;
                            ...
                            // ROS_INFO_STREAM("attempting merge with raw gap: (" << raw_gaps[i].RIdxPOV() << ", " << raw_gaps[i].RDistPOV() << ") to (" << raw_gaps[i].LIdxPOV() << ", " << raw_gaps[i].LDistPOV() << ")");
                            for (int j = (int) (simplified_gaps.size() - 1); j >= 0; j--)
                            {
                                // ROS_INFO_STREAM("on simplified gap " << j << " of " << simplified_gaps.size() << ": ");
                                // ROS_INFO_STREAM("points: (" << simplified_gaps[j].RIdxPOV() << ", " << simplified_gaps[j].RDistPOV() << ") to (" << simplified_gaps[j].LIdxPOV() << ", " << simplified_gaps[j].LDistPOV() << ")");
                                int start_idx = std::min(simplified_gaps[j].LIdxPOV(), raw_gaps[i].RIdxPOV());
                                int end_idx = std::max(simplified_gaps[j].LIdxPOV(), raw_gaps[i].RIdxPOV());
                            ...
                            if (last_mergable != -1) {
                                ...
                                // ROS_INFO_STREAM("merging simplified gap into (" << simplified_gaps.back().RIdxPOV() << ", " << simplified_gaps.back().RDistPOV() << ") to (" << simplified_gaps.back().LIdxPOV() << ", " << simplified_gaps.back().LDistPOV() << ")");
                            } else {
                                // ROS_INFO_STREAM("no merge, adding raw gap (swept, right): (" << raw_gaps[i].RIdxPOV() << ", " << raw_gaps[i].RDistPOV() << ") to (" << raw_gaps[i].LIdxPOV() << ", " << raw_gaps[i].LDistPOV() << ")");                            
                    ...
                    { // If current raw gap is radial
                        ...
                        if (std::abs(curr_rdist - simplified_gaps.back().RDistPOV()) < 0.2 && simplified_gaps.back().isAxial() && simplified_gaps.back().isRightPOVType())
                        {
                            ...
                            // ROS_INFO_STREAM("adding raw gap (radial): (" << raw_gaps[i].RIdxPOV() << ", " << raw_gaps[i].RDistPOV() << ") to (" << raw_gaps[i].LIdxPOV() << ", " << raw_gaps[i].LDistPOV() << ")");                            
                            simplified_gaps.push_back(raw_gaps[i]);

                            //std::cout << "adding raw gap from (" << raw_gaps[i].RIdxPOV() << ", " << raw_gaps[i].RDistPOV() << ") to (" << raw_gaps[i].LIdxPOV() << ", " << raw_gaps[i].LDistPOV() << ")" << std::endl;
                        }
                    }
                }
                else
