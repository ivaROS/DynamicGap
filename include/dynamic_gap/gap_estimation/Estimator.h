    ...
    */
    class Estimator 
    {
        public:
            ...
            std::vector<geometry_msgs::TwistStamped> interpIntermediateRbtVels_; /**< sequence of ego-robot velocities received since last model update */
            std::vector<geometry_msgs::TwistStamped> interpIntermediateRbtAccs_; /**< sequence of ego-robot accelerations received since last model update */     
            ...
            */
            void processEgoRobotVelsAndAccs(const ros::Time & tUpdate)
            {
                ...
                while (!interpIntermediateRbtVels_.empty() && tLastUpdate_ > interpIntermediateRbtVels_[0].header.stamp)
                {
                    interpIntermediateRbtVels_.erase(interpIntermediateRbtVels_.begin());
                    interpIntermediateRbtAccs_.erase(interpIntermediateRbtAccs_.begin());
                }
                ...
                {
                    interpIntermediateRbtVels_.erase(interpIntermediateRbtVels_.end() - 1);
                ...
                std::vector<geometry_msgs::TwistStamped> testInterpIntermediateRbtVels_ = interpIntermediateRbtVels_;
                std::vector<geometry_msgs::TwistStamped> testInterpIntermediateRbtAccs_ = interpIntermediateRbtAccs_;

                ROS_INFO_STREAM("incoming testInterpIntermediateRbtVels_: ");
                for (int i = 0; i < testInterpIntermediateRbtVels_.size(); i++)
                {
                    ROS_INFO_STREAM("   " << i << ": ");                
                ...
                ROS_INFO_STREAM("incoming testInterpIntermediateRbtAccs_: ");
                for (int i = 0; i < testInterpIntermediateRbtAccs_.size(); i++)
                {
                    ROS_INFO_STREAM("   " << i << ": ");                
                ...
                if (testInterpIntermediateRbtVels_.size() > 0 && interpIntermediateRbtAccs_.size() > 0)
                    interpolateIntermediateValues(testInterpIntermediateRbtVels_, interpIntermediateRbtAccs_);
                
                if (testInterpIntermediateRbtAccs_.size() > 0 && interpIntermediateRbtVels_.size() > 0)
                    interpolateIntermediateValues(testInterpIntermediateRbtAccs_, interpIntermediateRbtVels_);

                ROS_INFO_STREAM("outgoing testInterpIntermediateRbtVels_: ");
                for (int i = 0; i < testInterpIntermediateRbtVels_.size(); i++)
                {
                    ROS_INFO_STREAM("   " << i << ": ");                
                    ROS_INFO_STREAM("       header.stamp: " << testInterpIntermediateRbtVels_[i].header.stamp);
                ...
                ROS_INFO_STREAM("outgoing testInterpIntermediateRbtAccs_: ");
                for (int i = 0; i < testInterpIntermediateRbtAccs_.size(); i++)
                {
                    ROS_INFO_STREAM("   " << i << ": ");                
                    ROS_INFO_STREAM("       header.stamp: " << testInterpIntermediateRbtAccs_[i].header.stamp);
                ...
                */
                ...
                ROS_INFO_STREAM("outgoing interpIntermediateRbtVels_: ");
                for (int i = 0; i < interpIntermediateRbtVels_.size(); i++)
                {
                    ROS_INFO_STREAM("   " << i << ": ");                
                ...
                ROS_INFO_STREAM("outgoing interpIntermediateRbtAccs_: ");
                for (int i = 0; i < interpIntermediateRbtAccs_.size(); i++)
                {
                    ROS_INFO_STREAM("   " << i << ": ");                
